...
- We'll need to call `add_friendship()` 500 times. The reason been, is that each friendship is bidirectional, calling `add_friendship(1, 3)` will add both friendships `1:{3}` and `3:{1}` automatically. So for `100` users with an average of `10` friends each totals to `1000` then we divide that by `2` and we get `500`.

2. If you create 1000 users with an average of 5 random friends each, what percentage of other users will be in a particular user's extended social network? What is the average degree of separation between a user and those in his/her extended network?

- The percentage of other users that will be in a particular user's extended social network would be about `60%`.
- The average degree of separation between a user and those in his/her extended network would be about `15`.
